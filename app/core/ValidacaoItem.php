...
class ValidacaoItem {
    ...
    public function isMinimo($min, $msg = null, $erro = Validacao::ERRO_MINIMO) {
        if (is_int($min)) {
            ...
            throw new Exception("Você precisa definir um valor inteiro para a função de Validação Mínimo ");
        }
        return $this;
    }
    ...
    public function isIdadeMinima($idade, $msg = null, $erro = Validacao::ERRO_MINIMO) {
        if (is_int($idade)) {
            if (strlen($this->valor) < $idade) {
                if ($msg == null) {
                    $msg = "A idade mínima tem que ser 65 anos";
                }
                $this->erro[$erro] = $msg;
                $this->codigo_erro[$erro] = true;
            }
        } else {
            throw new Exception("Você precisa definir um valor inteiro para a função de Validação de Idade");
        }
        return $this;
    }

    public function isCelular($msg = null, $erro = Validacao::ERRO_CELULAR) {
        if (!validaCelular($this->valor)) {
            if ($msg == null) {
                $msg = "O campo " . $this->campo . " precisa ser um celular válido";
            }
            $this->erro[$erro]        = $msg;
            $this->codigo_erro[$erro] = true;
        }
